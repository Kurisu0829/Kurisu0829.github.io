...
## CSS
CSS涉及到的内容过多，这些内容适合需要用到的时候再去查阅，在这里记录一点基本的写法。

CSS样式表由一组规则组成，这些规则由Web浏览器解释，然后应用于文档中的相应元素，例如段落，标题等，具体结构为一个选择器和一个或多个声明，声明又由属性和对应的值组成。
...
选择器是CSS最重要的方面之一，因为它们用于选择网页上的元素，以便可以设置样式。您可以通过多种方式定义选择器，在这里有许多种写法：
``` css
/* 通用选择器，与页面上的每个单个元素匹配 */
* {
    margin: 0;
...
/* class类选择器，可用于选择具有class属性的任何HTML元素 */
.blue {
    color: blue;
  }

/* 后代选择器，当需要选择一个元素是另一个元素的后代时（也就是嵌套，可嵌套无数层），可以使用这些选择器 */
ul.menu li a {
...
/* 子选择器，子选择器只能用于选择作为某些元素的直接子元素的那些元素，嵌套一层 */
ul > li {
    list-style: square;
  }
ul > li ol {
